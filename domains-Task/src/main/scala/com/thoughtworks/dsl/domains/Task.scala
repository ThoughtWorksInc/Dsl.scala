...
/** The asynchronous task that supports exception handling, resource management,
  * and is stack-safe.
  *
  * @template
  * @example
  *   A [[Task]] can be created from `for`-comprehension, where
  *   [[keywords.Each]] can be used together to asynchronously iterate
  *   collections.
  *
  * For example, the above `concatenateRemoteData` downloads and concatenates
  ...
  * }}}
  *
  * A [[Task]] can be then converted to [[scala.concurrent.Future]] via
  * [[Task.toFuture]], in order to integrate into other frameworks.
  *
  * In this example, it's a `Future[Assertion]` required by
...
object Task extends TaskPlatformSpecificFunctions {
  ...
  @inline
  private def newBuilder[A, C](implicit
      factory: Factory[A, C]
  ): Builder[A, C] = {
    factory.newBuilder
  }
  ...
  /** Converts a [[Task]] to a [[scala.concurrent.Future]].
    *
    * @see
    *   [[keywords.Await]] for converting a [[scala.concurrent.Future]] to a
    *   [[Task]].
    */
